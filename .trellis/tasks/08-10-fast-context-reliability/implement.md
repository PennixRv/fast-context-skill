...
- [x] 以明确负责 047/048 的产品提交落盘。

## 2. Issue 045/046：有界搜索与共享预算

- [x] 新增集中命名的资源预算对象与默认上限，使用单调 deadline并合并外部 AbortSignal。
- [x] 将 PathGuard 遍历/regularFiles/glob 改为异步 typed 结果，限制 entries/directories/depth/files/matches/elapsed。
- [x] 修正 `**/` 的零目录与多目录语义，保持 deny/canonical root/symlink escape。
- [x] 将 executor 改为可取消无 shell异步子进程，逐块限制输出并等待 close。
- [x] 分批执行受 guard 约束的 `rg`，把 complete/truncated/failure 一致传给远端工具、公开 JSON 与最终状态。
- [x] 测试第 513 个文件、宽/深目录、2,500 空目录、无匹配 glob、4 x 3 共享 deadline、调用方取消和无遗留进程。
- [x] 记录小仓库可重复规模/延迟基准，更新 README/脚本契约。
- [x] 组合运行 path-guard、executor、core 与 CLI 测试并提交负责 045/046 的产品变更。

## 3. Issue 049：候选范围复核

- [x] 在 PathGuard 与统一预算内按固定文件版本验证候选范围，不 clamp。
- [x] 集中命名最大跨度，拒绝 start/end 越界、空文件、跨度超限和验证期间变化。
- [x] 覆盖无尾换行、最后一行、deny、根外、symlink escape 和至少一行可读性。
- [x] 固定本地 reason/status，更新文档区分路径、范围和最终语义核验。
- [x] 组合运行 core、PathGuard 与 CLI 测试并提交负责 049 的产品变更。
